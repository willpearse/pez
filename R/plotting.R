...
#' 
#' \code{barplot.comparative.comm} plots a barplot of a trait in a comparative community object
#' 
...
#' barplot(data, c("traitA", "traitB"))
#' }
#' @export
barplot.comparative.comm <- function(data, traits, scaling = FALSE, ranging = TRUE, yranging = NULL, tipsontop=TRUE,ceti = 1, csub = 0, f.phylog = 1/(1 + ncol(values)), clabel.leaves = 0, grid.col = gray(0.85), connect.col = "black",vert.line.col = grey(0.5), vert.line.lty = 1, bar.col = heat.colors(dim(values)[1], alpha = .8), spacing = 2.5,border.col = "black",draw.connecting.lines = FALSE, draw.grid = FALSE, draw.vert.line = TRUE, phylog.lwd=1, ...)
