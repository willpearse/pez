...
#' cells occuped by species A --> 10*p.migrate chance of migration)
#' @param env.lam Lambda value for Poisson distribution used to
#' distribute environmental quality; essentially the carrying capacity
#' (for each species separately) for that cell
#' @param abund.lam Lambda value for Poisson distribution used to
#' distribute initial abundances and abundance after migration
#' @param stoch.lam Lambda value for Poisson distribution of noise
#' added to the next step abundance calculation. With equal chance,
#' this is taken as either a positive or a negative number (see
#' details if you're confused as to why this is Poisson!)
...
#' environmental quality (carrying capacities)
#' @author Will Pearse
#' @export
...
#' speciate. A species can only speciate, migrate, or reproduce (etc.)
#' if it has individuals, so extinction is hanled automatically by the
#' other parameters.
#' @details Simulates species moving through a metacommunity. At each
#' time-step each cell's next abundance for each species is
#' env.quality - current.abundance + stochastic, and a species gets as
