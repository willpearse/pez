...
#
# 2. Make is so that the user can change the arguements in the ses. functions such as different null models. MRH does not know how to do this, is it with the "..." argument?
#
#
...
#'
#' @details Calculates various metrics of phylogenetic biodiversity that are categorized as \emph{dispersion} metrics by Pearse \emph{et al.} (2014)
# This calculates $SES_{MPD}$, $SES_{MNTD}$, $SES_{PD}$, INND, and D. All these are defined as dispersion metrics in Pearse et al., 
...
#' @references \code{sesmpd,sesmntd} Webb C.O. (2000). Exploring the phylogenetic structure of ecological communities: An example for rain forest trees. American Naturalist, 156, 145-155.
#' @references \code{sespd} Webb C.O., Ackerly D.D. & Kembel S.W. (2008). Phylocom: software for the analysis of phylogenetic community structure and trait evolution. Bioinformatics Applications Note, 24, 2098-2100.
#' @references \code{innd} Ness J.H., Rollinson E.J. & Whitney K.D. (2011). Phylogenetic distance can predict susceptibility to attack by natural enemies. Oikos, 120, 1327–1334.
#' @references \code{d} Fritz S.A. & Purvis A. (2010). Selectivity in Mammalian Extinction Risk and Threat Types: a New Measure of Phylogenetic Signal Strength in Binary Traits. Conservation Biology, 24, 1042-1051.
#
#' @examples \dontrun{
#' data(phylocom)
